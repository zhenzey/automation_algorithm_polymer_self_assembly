...
def difference(x):
    ...
    for i in np.arange(x.shape[0] - 1, -1, -1):
        distance = x[(iterations - i) % (x.shape[0]), 0]
...
if __name__ == "__main__":
    ...
    node0 = ovito.io.import_file("dump.A2B8_thin_film_with_wall_XN_100_35_ref", multiple_frames=True)
    ...
    bound = (np.array([2.5]), np.array([4.0]))

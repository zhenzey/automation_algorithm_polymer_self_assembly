...
def SingleChain(M, N, K):
    ...
    AtomTypes = 4
    BondTypes = 1
    AtomNo = 4 * M + 8 * q + 6 * r # +360*36+3600
    BondNo = 3 * M  # +3600
    ...
    for m in range(0, M):
        for n in range(1, N + 1):
            AtomList.append([n + m * 4, m + 1, 1, m // 108 * 0.5 + 0.95 * n, m % 108 * 0.5, 0.15 * n])
            if (n != 4):
                BondList.append([n + m * 3, 1, n + m * 4, n + 1 + m * 4])
        for n in range(N + 1, 5):
            AtomList.append([n + m * 4, m + 1, 2, m // 108 * 0.5 + 0.95 * n , m % 108 * 0.5, 0.15 * n])
            if (n != 4):
                BondList.append([n + m * 3, 1, n + m * 4, n + 1 + m * 4])
    ...
    for j in range(8):
        for i in range(j * q + 1, (j + 1) * q + 1):
            AtomList.append([M * 4 + i, np.where(j < 4, M + 1, M + 2), np.where(j < 4, 3, 4), np.where(j % 2 == 0, i % p * 0.5 + (i // p) % 2 * 0.25, i % p * 0.5 + (i // p) % 2 * 0.25 + 0.25), np.where(j % 2 == 0, i // p * 0.43, i // p * 0.43 + 0.14), np.where(j < 4, -0.50 - 0.41 * j, K + 0.5 + (j - 4) * 0.41)])
    ...
    f = open(str(M) + 'A' + str(N) + 'B' + str(4 - N) + '.dat', 'w')
...
N = [1]#number of A bead
K = [3, 5, 6, 8, 10]#height atz axis
for k in K:
    for n in N:
        SingleChain(k * 43 * 5, n, k) #43 = box size ** 2 / chain length

...
def SingleChain(M, N, K, box, chain_length, post_array):
    ...
    # lay particles
    a = int(density * xsize * z / chain_length * 2)
    b = xsize / chain_length * 0.3
    for m in range(0, M):
...
box = [0.0, number[0]* distance , 0.0, number[1] * distance * 1.732 / 2, 0.0, 25.0]
xsize = box[1] - box[0]
ysize = box[3] - box[2]
z = box[5] - box[4]
density = 5.0
